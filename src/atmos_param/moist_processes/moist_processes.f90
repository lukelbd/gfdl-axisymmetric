...
use       constants_mod, only: CP_AIR, GRAV, RDGAS, RVGAS, HLV, KAPPA
...
! ============================READ VAPOR_NML data============================
       if ( file_exist('input.nml')) then
         ...
         ierr=1; 
         do while (ierr /= 0)
            read  (unit, nml=vapor_nml, iostat=io, end=11)
            ierr = check_nml_error(io,'vapor_nml')
         enddo
  
  11     call close_file (unit)
!--------- namelist to standard log ------------
      if ( mpp_pe() == mpp_root_pe() ) &
