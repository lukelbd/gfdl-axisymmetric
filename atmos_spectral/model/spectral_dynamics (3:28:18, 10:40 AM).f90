...
integer :: id_vt_mean, id_uv_mean, id_mass_u, id_mass_v
...
flux_mass_uv_int = 0
if (apply_fluxes) then
  call pres_interp(pres_data_grid,flux_mass_div,p_full,flux_mass_div_int,.false.)
  call pres_interp(pres_data_grid,flux_mass_uv,p_full,flux_mass_uv_int,.false.)
endif
...
!regular tendencies + mom forcing
do j = js,je
  do i=is,ie
    do k=1,num_levels
...
'dy((uv))', axes_3d_full, Time, 'dy(meridional wind * zonal wind)','(m**2/s**2)',range=(/-vrange(2)**2,vrange(2)**2/))
...
real, dimension(is:ie, js:je, num_levels)   :: ln_p_full, p_full, z_full, work, coslat, work_convergence, & 
                                               pfull_grid, mse, dse, work_interp, work_convergence_sum
...
if(id_ov > 0) then
  ...
  call flux_convergence_vertical(work, work_convergence, p_full)
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
...
if (id_vmse > 0 .or. id_vdse > 0 .or. id_vmse_eddy > 0 .or. id_vdse_eddy > 0) then
