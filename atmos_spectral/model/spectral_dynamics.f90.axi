...
use         transforms_mod, only: transforms_init,         transforms_end,            &
                                  ...
                                  spectral_domain,         get_deg_lon, get_deg_lat, compute_lat_deriv_cos
...
public :: get_axis_id, spectral_diagnostics, get_initial_fields, interpol
...
integer :: id_vt, id_vq, id_ou, id_ov, id_ot, id_oq
...
real    :: damping_coeff       = 1.15740741e-4, & ! (one tenth day)**-1
     ...
     reference_sea_level_press =  101325., &
	   mom_forcing_p       = 300., &
           mom_forcing_lat     = 20., &
	   mom_forcing_p_width = 50., &
           mom_forcing_lat_width = 3., &
	   mom_forcing_mag       = 1.157e-5
!===============================================================================================
...
namelist /spectral_dynamics_nml/ use_virtual_temperature, damping_option,                            &
                                 ...
                                 print_interval, num_steps, mom_forcing_p, mom_forcing_lat,          &
                                mom_forcing_p_width, mom_forcing_lat_width, mom_forcing_mag,         &
                                mom_forcing, mom_forcing_symmetric, apply_fluxes, do_random_shift
...
real   , dimension( num_levels               ) :: pres_data, x_int, y_int, t_int, q_int, q_c_int, q_l_int, t_c_int, t_l_int
real   , dimension(js:je, num_levels         ) :: t_int_y, q_int_y, x_int_y, y_int_y, q_l_int_y, t_l_int_y, q_c_int_y, t_c_int_y
...
real   , dimension(is:ie, js:je                         ) :: dt_psg_tmp!, flux_q_surf, flux_t_surf, &
                                                          !flux_u_surf, flux_v_surf, rho
real, dimension(is:ie, js:je, num_levels             ) :: dt_ug_tmp, dt_vg_tmp, dt_tg_tmp, dt_mom, flux_x, flux_y, flux_t, flux_q
...
real    :: rand_num_lat, rand_num_pres
...
if (apply_fluxes) then
    ...
    call read_data('/home/nadavis/moist_gcm/fluxes.nc', 'forcing_x', flux_x(:,:,:), grid_domain)
    call read_data('/home/nadavis/moist_gcm/fluxes.nc', 'forcing_y', flux_y(:,:,:), grid_domain)
    call read_data('/home/nadavis/moist_gcm/fluxes.nc', 'forcing_t', flux_t(:,:,:), grid_domain)
    call read_data('/home/nadavis/moist_gcm/fluxes.nc', 'forcing_q', flux_q(:,:,:), grid_domain)
    call read_data('/home/nadavis/moist_gcm/fluxes.nc', 'pres', pres_data, no_domain=.true.)

endif

!test_forcing=pres_data(1)
! write(*,'(e16.8)') test_forcing
!  call error_mesg('spectral_dynamics','top level loaded = '//ch_tmp1, NOTE)
...
call random_number(rand_num_pres)
call random_number(rand_num_lat)

!changes of +/- 2.5 deg - needs to be in radians as asin output is in radians (mean deviation in either direction is 1.25)
rand_num_lat=(rand_num_lat-0.5)*4;
!changes of +/- 15 hPa
rand_num_pres=(rand_num_pres-0.5)*20;

call get_deg_lat(lat)

if (apply_fluxes) then

    if (do_random_shift) then
      do k = 1,num_levels
          call interpol(size(lat,1), lat, flux_t(1,:,k), size(lat,1), lat-rand_num_lat, t_int_y(:,k))
      enddo
    else
      t_int_y=flux_t(1,:,:)
    endif
    ...
    do j=js,je
        call interpol(size(pres_data,1), pres_data, t_int_y(j,:), size(p_full,3), p_full(1,j,:)-rand_num_pres, t_int)
        do k=1,num_levels
            do i=is,ie
                dt_tg_tmp(i,j,k) = dt_tg_tmp(i,j,k) + t_int(k)
            enddo
        enddo
...
if (apply_fluxes) then
    ...
    if (do_random_shift) then
      do k = 1,num_levels
          call interpol(size(lat,1), lat, flux_y(1,:,k), size(lat,1), lat-rand_num_lat, y_int_y(:,k))
          call interpol(size(lat,1), lat, flux_x(1,:,k), size(lat,1), lat-rand_num_lat, x_int_y(:,k))
      enddo
    else
      x_int_y=flux_x(1,:,:)
      y_int_y=flux_y(1,:,:)
    ...
    !random pressure variation
    do j= js,je
        call interpol(size(pres_data,1), pres_data, y_int_y(j,:), size(p_full,3), p_full(1,j,:), y_int)
        call interpol(size(pres_data,1), pres_data, x_int_y(j,:), size(p_full,3), p_full(1,j,:), x_int)
        do k=1, num_levels
            do i=is,ie
                dt_vg_tmp(i,j,k) = dt_vg_tmp(i,j,k) + y_int(k)
                dt_ug_tmp(i,j,k) = dt_ug_tmp(i,j,k) + x_int(k)
            enddo
        enddo
    enddo
endif
...
if (apply_fluxes) then
    if (do_random_shift) then
      do k = 1,num_levels
          call interpol(size(lat,1), lat, flux_q(1,:,k), size(lat,1), lat-rand_num_lat, q_int_y(:,k))
      enddo
    else
      q_int_y=flux_q(1,:,:)
    endif
    ...
    do j=js,je
        call interpol(size(pres_data,1), pres_data, q_int_y(j,:), size(p_full,3), p_full(1,j,:)-rand_num_pres, q_int)
        do k=1,num_levels
            test_forcing = grid_tracers(1,j,k,current,nhum) + (q_int(k) * delta_t)
            if (test_forcing .gt. 0) then
              do i=is,ie
                  dt_tracers_tmp(i,j,k,nhum) = dt_tracers_tmp(i,j,k,nhum) + q_int(k)
              enddo
            endif
        enddo
    enddo
...
enddo
if (axi_test .eq. 1) call error_mesg('spectral_dynamics','no longer axisymmetric',NOTE)
...
!===================================================================================
subroutine flux_average_3d(flux1,flux2,eddy)

real, intent(in), dimension(is:ie, js:je, num_levels)  :: flux1, flux2
real, intent(out), dimension(is:ie, js:je, num_levels) :: eddy
integer :: k

do k=1,num_levels
  call flux_average(flux1(:,:,k),flux2(:,:,k),eddy(:,:,k))
enddo

end subroutine flux_average_3d
!===================================================================================
subroutine flux_average(flux1,flux2,eddy)

real, intent(in), dimension(is:ie, js:je)  :: flux1, flux2
real, intent(out), dimension(is:ie, js:je) :: eddy
real :: average1, average2, average
integer :: i,j

!calculate eddy fluxes as [u'v'] = [uv] - [u][v]
do j=js,je
  average1=sum(flux1(:,j))/real(size(flux1(:,j),1))
  average2=sum(flux2(:,j))/real(size(flux2(:,j),1))
  average=sum(flux1(:,j)*flux2(:,j))/real(size(flux1,1))
  do i=is,ie
    eddy(i,j)=average-(average1*average2)
  enddo
enddo
...
!===================================================================================
subroutine interpol ( nd, xd, yd, ni, xi, yi )
...
!orientation: (1) is TOA, (end) is surface
do i = 1, ni
    !is the desired point above our TOA point?
    !set it to the known TOA value
    if (xi(i) .le. xd(1)) then
        yi(i)=yd(1)
    !is requested data point below our surface point?
    !set it to the known surface value
    elseif (xd(nd) .le. xi(i)) then
        yi(i)=yd(nd)
    else
        do k = 2, nd
            ! we've identified our interpolation points
            if ( xd(k-1) .lt. xi(i) .and. xi(i) .lt. xd(k) ) then
                t=(yd(k)-yd(k-1))/(xd(k)-xd(k-1))
                yi(i)=yd(k-1)+(xi(i)-xd(k-1))*t
            endif
        enddo
    endif
enddo
