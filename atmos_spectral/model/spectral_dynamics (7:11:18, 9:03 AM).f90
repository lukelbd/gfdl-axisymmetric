...
use          constants_mod, only: rdgas, rvgas, grav, cp_air, omega, radius, pi, hlv
...
use         transforms_mod, only: transforms_init,         transforms_end,            &
                                  ...
                                  compute_lat_deriv_cos,   get_cos_lat,               &
                                  divide_by_cos2
...
public :: get_axis_id, spectral_diagnostics, get_initial_fields, flux_average
!===============================================================================================
...
integer :: id_udt_damp, id_vdt_damp, id_tdt_damp, id_dse, id_mse
...
subroutine spectral_dynamics(Time, psg_final, ug_final, vg_final, tg_final, tracer_attributes, grid_tracers_final, &
                             ...
                             dt_ug_damp, dt_vg_damp, dt_tg_damp)
...
real   , dimension(is:ie, js:je, num_levels, num_tracers ) :: dt_tracers_eddy
real   , dimension( num_levels               ) :: pres_data
...
real, dimension(is:ie, js:je, num_levels             ) :: dt_ug_tmp, dt_vg_tmp, dt_tg_tmp, dt_mom
real, dimension(is:ie, js:je, num_levels             ) :: flux_x, flux_y, flux_t, flux_q, flux_mass_div
...
real, intent(out), dimension(is:ie, js:je, num_levels) :: dt_ug_damp, dt_vg_damp, dt_tg_damp
...
if (apply_fluxes) then
  call read_data(input_file, 'forcing_x', flux_x(:,:,:), grid_domain)
  call read_data(input_file, 'forcing_y', flux_y(:,:,:), grid_domain)
...
do j=js,je
  do i=is,ie
    do k=1,num_levels
        dt_ug_tmp(i,j,k) = dt_ug_tmp(i,j,k) + (vorg(i,j,k) + coriolis(j))*vg(i,j,k,current)
        dt_vg_tmp(i,j,k) = dt_vg_tmp(i,j,k) - (vorg(i,j,k) + coriolis(j))*ug(i,j,k,current)
    enddo
  enddo
enddo
...
if (apply_fluxes) then
  ...
  enddo
  if (axi_test .eq. 1) call error_mesg('spectral_dynamics','WARNING: Simulation is no longer axisymmetric',NOTE)
endif
...
integer, dimension(2) :: axes_2d_full
integer :: id_lonb, id_latb, id_phalf, id_lon, id_lat, id_pfull, id_preal
...
id_udt_damp =  register_diag_field(mod_name, 'udt_damp',  axes_3d_full, Time, 'Zonal wind tend from horiz diff',      'm/s**2')
id_vdt_damp =  register_diag_field(mod_name, 'vdt_damp',  axes_3d_full, Time, 'Merid wind tend from horiz diff',      'm/s**2')
id_tdt_damp =  register_diag_field(mod_name, 'tdt_damp',  axes_3d_full, Time, 'Temperature tend from horiz diff',      'm/s**2')
...
id_u_pg   = register_diag_field(mod_name, &
      'ucomp_pgrid',   axes_3d_full,       Time, 'zonal wind component',         'm/sec',      range=vrange)

id_v_pg   = register_diag_field(mod_name, &
      'vcomp_pgrid',   axes_3d_full,       Time, 'meridional wind component',    'm/sec',      range=vrange)

id_uu  = register_diag_field(mod_name, &
'uu',axes_3d_full,       Time, 'zonal wind squared',           '(m/sec**2)', range=(/0.,vrange(2)**2/))

id_vv  = register_diag_field(mod_name, &
'dy(vv)',axes_3d_full, Time, 'dy(meridional wind squared)', '(m/sec**2)', range=(/-vrange(2)**2,vrange(2)**2/))

id_vv_full  = register_diag_field(mod_name, &
'vv',axes_3d_full, Time, 'meridional wind squared', '(m/sec)**2', range=(/0.,vrange(2)**2/))

id_uv  = register_diag_field(mod_name, &
'dy(uv)', axes_3d_full, Time, 'dy(zonal wind * meridional wind)', '(m/sec**2)', range=(/-vrange(2)**2,vrange(2)**2/))

id_vq = register_diag_field(mod_name, &
'dy(vq)', axes_3d_full, Time, 'dy(meridional wind * sphum)','(1/sec)',range=(/-vrange(2)**2,vrange(2)**2/))

id_vt = register_diag_field(mod_name, &
'dy(vt)', axes_3d_full, Time, 'dy(meridional wind * temperature)','(K/sec)',range=(/-vrange(2)**2,vrange(2)**2/))

id_ov  = register_diag_field(mod_name, &
'dp(ov)',axes_3d_full, Time, 'dp(omega * meridional wind)','(m/sec**2)', range=(/-vrange(2)**2,vrange(2)**2/))

id_ou  = register_diag_field(mod_name, &
'dp(ou)', axes_3d_full, Time, 'dp(zonal wind * omega)', '(m/sec**2)', range=(/-vrange(2)**2,vrange(2)**2/))

id_oq = register_diag_field(mod_name, &
'dp(oq)', axes_3d_full, Time, 'dp(omega* sphum)','(1/sec)',range=(/-vrange(2)**2,vrange(2)**2/))

id_ot = register_diag_field(mod_name, &
'dp(ot)', axes_3d_full, Time, 'dp(omega * temperature)','(K/sec)',range=(/-vrange(2)**2,vrange(2)**2/))

id_ot_full = register_diag_field(mod_name, &
'ot', axes_3d_full, Time, 'omega * temperature','(Pa K/sec)',range=(/-vrange(2)**2,vrange(2)**2/))

id_mass  = register_diag_field(mod_name, &
'eddy_mass_div', axes_3d_full, Time, 'eddy mass convergence: div','(Pa m/s)',range=(/-vrange(2)**2,vrange(2)**2/))

id_mass_u  = register_diag_field(mod_name, &
'eddy_mass_u', axes_3d_full, Time, 'eddy mass convergence: uwnd','(Pa m/s)',range=(/-vrange(2)**2,vrange(2)**2/))
...
id_t_pg   = register_diag_field(mod_name, &
      'temp_pgrid',    axes_3d_full,       Time, 'temperature',                  'deg_k',      range=trange)
...
id_pfullzm   = register_diag_field(mod_name, &
      'pres_zm',  axes_3d_full,       Time, '','hPa')

id_ot_full_mean = register_diag_field(mod_name, &
'ot_mean', axes_3d_full, Time, 'omega * temperature','(Pa K/sec)',range=(/-vrange(2)**2,vrange(2)**2/))

id_vt_mean = register_diag_field(mod_name, &
'vdy(t)', axes_3d_full, Time, 'meridional temperature advection','(K/sec)',range=(/-vrange(2)**2,vrange(2)**2/))

id_uv_mean = register_diag_field(mod_name, &
'vdy(u)', axes_3d_full, Time, 'meridional momentum advection','(m**2/s**2)',range=(/-vrange(2)**2,vrange(2)**2/))

id_uo_mean = register_diag_field(mod_name, &
'omegadp(u)', axes_3d_full, Time, 'vertical momentum advection','(m**2/s**2)',range=(/-vrange(2)**2,vrange(2)**2/))

id_to_mean = register_diag_field(mod_name, &
'omegadp(t)', axes_3d_full, Time, 'vertical temperature advection','(m**2/s**2)',range=(/-vrange(2)**2,vrange(2)**2/))

id_dse = register_diag_field(mod_name, &
'dse', axes_3d_full, Time, 'dse','(J/kg)')

id_mse = register_diag_field(mod_name, &
'mse', axes_3d_full, Time, 'mse','(J/kg)')

id_vmse = register_diag_field(mod_name, &
'vmse', axes_3d_full, Time, 'v*mse','(m*J/kg/s)')

id_vdse = register_diag_field(mod_name, &
'vdse', axes_3d_full, Time, 'v*dse','(m*J/kg/s)')

id_vmse_eddy = register_diag_field(mod_name, &
'vmse_eddy', axes_3d_full, Time, 'v*mse eddy','(m*J/kg/s)')

id_vdse_eddy = register_diag_field(mod_name, &
'vdse_eddy', axes_3d_full, Time, 'v*dse eddy','(m*J/kg/s)')
...
!===================================================================================
subroutine spectral_diagnostics(Time, p_surf, u_grid, v_grid, t_grid, wg_full, tr_grid, dt_ug_damp, dt_vg_damp, dt_tg_damp)
...
!Zonal-mean pressure field
call pressure_variables(phalf_1d, ln_phalf_1d, pfull_1d, ln_pfull_1d, reference_sea_level_press)  
do i=is,ie
  do j=js,je
    pfull_grid(i,j,:)=pfull_1d
  enddo
enddo
call zonal_average(p_full,pfull_zm)

if(id_pfullzm > 0) used = send_data(id_pfullzm, pfull_zm, Time)
...
if(id_u_pg   > 0 ) then
   call pres_interp(p_full,u_grid,pfull_grid,u_int,.false.)
   used = send_data(id_u_pg,   u_int, Time)
endif
if(id_v_pg   > 0 ) then
  call pres_interp(p_full,v_grid,pfull_grid,v_int,.false.)
  used = send_data(id_v_pg,   v_int, Time)
endif
if(id_t_pg   > 0 ) then
  call pres_interp(p_full,t_grid,pfull_grid,t_int,.false.)    
  used = send_data(id_t_pg,   t_int, Time)
endif

if(id_mass > 0 .or. id_mass_u > 0 .or. id_mass_v > 0) then  
  ln_psg = alog(p_surf)
  call trans_grid_to_spherical(ln_psg, ln_ps)
  call compute_pressure_gradient(ln_ps, p_surf, dx_psg, dy_psg)
endif
...
if(id_uu > 0) then
  call flux_average(u_grid,u_grid,work)
  call pres_interp(pfull_zm,work,pfull_grid,work_interp,.true.)
  used = send_data(id_uu, work_interp, Time)
endif
if(id_mass_u > 0) then
  do k=1,num_levels
    work(:,:,k) = dx_psg
...
if(id_vv > 0) then
  ...
  call flux_convergence_horiz(work, work_convergence)
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
  used = send_data(id_vv, work_interp, Time)
endif
if(id_uv > 0) then
  call flux_average(u_grid,v_grid,work)
  work = work * coslat
  call flux_convergence_horiz(work, work_convergence)
  work_convergence = work_convergence / coslat
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
  used = send_data(id_uv, work_interp, Time)
endif

if(id_uv_mean > 0) then
  call zonal_average(u_grid,work)
  call zonal_average(v_grid,work_2)
  work = work / coslat
  call flux_convergence_horiz(work,work_convergence)
  work_convergence = work_2 * work_convergence * coslat
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
  used = send_data(id_uv_mean, work_interp, Time)
endif
if(id_uo_mean > 0) then
  call zonal_average(u_grid,work)
  call zonal_average(wg_full,work_2)
  call flux_convergence_vertical(work,work_convergence, pfull_zm)
  work_convergence = wg_full * work_convergence
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
  used = send_data(id_uo_mean, work_interp, Time)
endif

if(id_vt_mean > 0) then
  call zonal_average(t_grid,work)
  call zonal_average(v_grid,work_2)
  work = work / coslat
  call flux_convergence_horiz(work,work_convergence)
  work_convergence = work_2 * work_convergence * coslat
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
  used = send_data(id_vt_mean, work_interp, Time)
endif
if(id_to_mean > 0) then
  call zonal_average(wg_full,work_2)
  call zonal_average(t_grid,work)
  call flux_convergence_vertical(work,work_convergence, pfull_zm)
  work_convergence = work_2 * work_convergence
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
  used = send_data(id_to_mean, work_interp, Time)
endif

if(id_vt > 0) then
  call flux_average(t_grid,v_grid,work)
  call flux_convergence_horiz(work, work_convergence)
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
  used = send_data(id_vt, work_interp, Time)
endif
if(id_vq > 0) then
  call flux_average(tr_grid(:,:,:,nhum),v_grid,work)
  call flux_convergence_horiz(work, work_convergence)
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
  used = send_data(id_vq, work_interp, Time)
endif
if(id_ov > 0) then
  call flux_average(wg_full,v_grid,work)
  call flux_convergence_vertical(work, work_convergence, pfull_zm)
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
  used = send_data(id_ov, work_interp, Time)
endif
if(id_ou > 0) then
  call flux_average(wg_full,u_grid,work)
  call flux_convergence_vertical(work, work_convergence, pfull_zm)
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
  used = send_data(id_ou, work_interp, Time)
endif
if(id_ot > 0) then
  call flux_average(wg_full,t_grid,work)
  call flux_convergence_vertical(work, work_convergence, pfull_zm)
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
  used = send_data(id_ot, work_interp, Time)
endif
if(id_ot_full > 0) then
  call flux_average(wg_full,t_grid,work)
  work = work / pfull_zm
  call pres_interp(pfull_zm,work,pfull_grid,work_interp,.true.)
  used = send_data(id_ot_full, work_interp, Time)
endif
if(id_vv_full > 0) then
  call flux_average(v_grid,v_grid,work)
  call pres_interp(pfull_zm,work,pfull_grid,work_interp,.true.)
  used = send_data(id_vv_full, work_interp, Time)
endif
if(id_oq > 0) then
  call flux_average(wg_full,tr_grid(:,:,:,nhum),work)
  call flux_convergence_vertical(work, work_convergence, pfull_zm)
  call pres_interp(pfull_zm,work_convergence,pfull_grid,work_interp,.true.)
  used = send_data(id_oq, work_interp, Time)
endif
if(id_tt > 0) then
  work = t_grid**2
...
if (id_vmse > 0 .or. id_vdse > 0 .or. id_vmse_eddy > 0 .or. id_vdse_eddy > 0 .or. id_dse > 0 .or. id_mse > 0) then
  dse = cp_air*t_grid + grav*z_full 
  mse = dse + hlv*tr_grid(:,:,:,1)
endif

if (id_dse > 0) then
  used=send_data(id_dse,dse,Time)
endif
if (id_mse > 0) then
...
if (id_vmse > 0) then
  work = v_grid * mse
  call pres_interp(pfull_zm,work,pfull_grid,work_interp,.true.)
  used = send_data(id_vmse, work_interp, Time)
endif
if (id_vdse > 0) then
  work = v_grid * dse
  call pres_interp(pfull_zm,work,pfull_grid,work_interp,.true.)
  used = send_data(id_vdse, work_interp, Time)
endif
if (id_vmse_eddy > 0) then
  call flux_average(v_int,mse,work)
  call pres_interp(pfull_zm,work,pfull_grid,work_interp,.true.)
  used = send_data(id_vmse_eddy, work_interp, Time)
endif
if (id_vdse_eddy > 0) then
  call flux_average(v_int,dse,work)
  call pres_interp(pfull_zm,work,pfull_grid,work_interp,.true.)
  used = send_data(id_vdse_eddy, work_interp, Time)
endif
if(id_ot_full_mean > 0) then
  work = wg_full * t_int / p_full
  call zonal_average(work,work_2)
  call pres_interp(pfull_zm,work_2,pfull_grid,work_interp,.true.)
  used = send_data(id_ot_full_mean, work_interp, Time)
endif
...
!===================================================================================
subroutine zonal_average(field,field_zm)
real, intent(in), dimension(is:ie,js:je,num_levels)  :: field
real, intent(out), dimension(is:ie,js:je,num_levels) :: field_zm
integer                          :: k, j, i
real                             :: average
...
real, intent(in), dimension(is:ie, js:je, num_levels)  :: flux
...
!===================================================================================
subroutine flux_convergence_vertical(flux,flux_convergence,p_full)

real, intent(in), dimension(is:ie, js:je, num_levels)  :: flux, p_full
real, intent(out), dimension(is:ie, js:je, num_levels) :: flux_convergence
integer :: k
...
enddo
flux_convergence(:,:,1) = ((flux(:,:,2)-flux(:,:,1))) / (p_full(:,:,2)-p_full(:,:,1))
...
real, intent(in), dimension(is:ie, js:je, num_levels)         :: field, pres, pres_grid
real, intent(out),dimension(is:ie, js:je, num_levels)         :: field_interp
logical, intent(in)                                    :: zonal_mean 
integer                                                :: j, k, l, i, is_h, ie_h, ii, jj, n, ki
integer, dimension(3)                                  :: k_array 
logical                                                :: do_interp
real, dimension(num_levels)                            :: diff_pres
real, dimension(3)                                     :: pres_i,field_i
real                                                   :: L1,L2,L3, dp
character(len=8) :: outputi
...
do k=1,num_levels
  do j=js,je
    do i=is_h,ie_h
      ...
      !find the closest pressure index in the data
      diff_pres = pres_grid(i,j,k) - pres(i,j,:)
      do l=1,num_levels
        diff_pres(l) = abs(diff_pres(l))
      enddo
      ki = minloc(diff_pres,1)

      !special case of index being outside the bounds of the grid
      if (ki .eq. 1) then
        if (pres(i,j,ki) .gt. pres_grid(i,j,k)) then
          do_interp = .false.
        else 
          k_array = (/1, 2, 3/)
        endif
      elseif (ki .eq. num_levels) then
        if (pres(i,j,ki) .lt. pres_grid(i,j,k)) then
          do_interp = .false.
        else 
          k_array = (/num_levels-2, num_levels-1, num_levels/)
        endif
      else
        k_array = (/ki-1, ki, ki+1/)
      endif

      if (do_interp) then
        pres_i=pres(i,j,k_array)
        field_i=field(i,j,k_array)

        L1 = ((pres_grid(i,j,k)-pres_i(2))*(pres_grid(i,j,k)-pres_i(3)))/((pres_i(1)-pres_i(2))*(pres_i(1)-pres_i(3)))

        L2 = ((pres_grid(i,j,k)-pres_i(1))*(pres_grid(i,j,k)-pres_i(3)))/((pres_i(2)-pres_i(1))*(pres_i(2)-pres_i(3)))

        L3 = ((pres_grid(i,j,k)-pres_i(1))*(pres_grid(i,j,k)-pres_i(2)))/((pres_i(3)-pres_i(1))*(pres_i(3)-pres_i(2)))
       
        field_interp(i,j,k) = field_i(1) * L1 + field_i(2) * L2 + field_i(3) * L3
      else
        field_interp(i,j,k) = field(i,j,ki)
      endif
    enddo
  enddo
enddo
