...
use spectral_dynamics_mod, only: interpol
...
use transforms_mod, only:  get_grid_domain, grid_domain, get_deg_lat
...
real    :: scale_factor     = 0.56
real    :: scale_width      = 25.0
...
namelist /simple_surface_nml/ z_ref_heat, z_ref_mom,             &
			      ...
			      deltaT, do_sst_clamp, scale_factor, scale_width, apply_fluxes
...
real, dimension(size(Atm%t_bot,1), size(Atm%t_bot,2)) :: &
       ...
       flux_t_surf, flux_q_surf, flux_u_surf, flux_v_surf, umag, vmag, delta_t, factor

real, dimension(size(Atm%t_bot,2))  :: interp_field_v, interp_field_u, interp_field_q, interp_field_t, deg_lat
...
!    call compute_ocean_roughness (mask, flux_u, flux_v,   & ! Fez
   ...
   flux_v_surf = 0
    if (apply_fluxes) then
      call set_domain(grid_domain)
      call read_data('/home/nadavis/moist_gcm/fluxes.nc', 'forcing_taux',   flux_u_surf(:,:), grid_domain)
      call read_data('/home/nadavis/moist_gcm/fluxes.nc', 'forcing_tauy',   flux_v_surf(:,:), grid_domain)

        call get_deg_lat(deg_lat)
        call random_number(rand_num_lat)
        rand_num_lat=(rand_num_lat-0.5)*2;

        !shift flux field around randomly to prevent singularities
        call interpol(size(deg_lat,1), deg_lat, flux_u_surf(1,:), size(deg_lat,1), deg_lat-rand_num_lat, interp_field_u)
        call interpol(size(deg_lat,1), deg_lat, flux_v_surf(1,:), size(deg_lat,1), deg_lat-rand_num_lat, interp_field_v)

        do i=1,size(flux_u_surf,1)
            flux_u_surf(i,:)=interp_field_u
            flux_v_surf(i,:)=interp_field_v
...
!                     dt, mask, glacier)
   ...
   call surface_flux (Atm%t_bot, Atm%q_bot, Atm%u_bot, Atm%v_bot,        & ! Lima
                      ...
                      mask, flux_u_surf, flux_v_surf)                                               ! Required argument, intent(in). Looks like it should be .true. everywhere.
...
integer :: i
    ...
    call set_domain(grid_domain)
    call read_data('/home/nadavis/moist_gcm/sst.nc', 'ts_field',   sst_clamp(:,:), grid_domain)
