...
p_center = (120- 90 * cos(0.7*lat(1,:))**4)*100
p_width = (35+25*sin(1*lat(1,:))**6)*100
...
! modif df 1-23-04: changing profile of IR absorber
  ...
  do j=1,size(lat,2)
    solar_tau(j,k) = (1-frac_oz)*solar_tau_0(j)*(p_half(1,j,k)/p00)**4 &
        + solar_tau_0(j)*frac_oz*((4/80)*sqrt(3.1416)*(erf(-((p_half(1,j,k)-250)/80)**2)+1)*(sin(1.1*lat(1,j))**10) &
        +0.5*sqrt(3.1416)*(erf(-(((p_half(1,j,k)-p_center(j))/p_width(j)))**2)+1))
  end do
end do
